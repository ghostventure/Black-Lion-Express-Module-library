...
try{
 ...
 await page.getByRole('link',{name:'Create a LionMax account'}).click();await page.getByLabel('Your name').fill('Browser Owner');await page.getByLabel('Choose a username').fill('browser.owner');await page.getByLabel('Create a password',{exact:true}).fill('A browser test passphrase 123!');await page.getByRole('checkbox', {name: /I understand/}).check();await page.getByRole('button',{name:'Create account'}).click();const token=await page.locator('#saved-token').textContent();assert.match(token,/^[A-Za-z0-9]{7}$/);
